...
** TODO [#A] When display only #A, unfold all
...
115 sub to-markup ($text is rw) {   
** TODO why it's necessary to write .Array ?
193 $orgmode~=" :" ~ join(':',$.tags.Array) ~ ':' if $.tags;
** TODO doesn't work, why ?
302 # $_.iter ne $iter
** TODO to improve
374 method search-indice($task) { # it's the indice on my tree, not Gtk::Tree
** TODO to remove, improve grammar/AST
508 sub demo_procedural_read($name) {
...
742 "_Ok", GTK_RESPONSE_OK,    
** TODO BUG Cannot look up attributes in a AppSignalHandlers type object
409 #:parent($!top-window),   
** TODO destroy ?
422 $dialog.gtk-widget-hide;
